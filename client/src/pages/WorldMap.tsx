...
import { ZoomIn, ZoomOut, Filter, Globe, DollarSign, Calendar, Info } from 'lucide-react';
...
const continentMap: Record<string, string> = {
    ...
    pa: 'Ameryka Północna',
    // Ameryka Południowa
    br: 'Ameryka Południowa',
    ar: 'Ameryka Południowa',
    co: 'Ameryka Południowa',
    cl: 'Ameryka Południowa',
    pe: 'Ameryka Południowa',
    ve: 'Ameryka Południowa',
    ec: 'Ameryka Południowa',
    bo: 'Ameryka Południowa',
    // Afryka
    eg: 'Afryka',
    za: 'Afryka',
    ma: 'Afryka',
...
const WorldMap = () => {
    ...
    const completedCountryCodes = new Set(
        filteredTrips
            ...
            .map((t) => (t.countryCode ? alpha2ToNumeric[t.countryCode.toLowerCase()] : null))
            .filter(Boolean),
    );
                    ...
                    <div className="bg-gray-200/80 dark:bg-[#262626]/95 backdrop-blur-lg w-full max-w-4xl lg:max-w-[1500px] min-h-[75vh] p-6 py-10 sm:p-10 lg:p-12 rounded-3xl lg:rounded-[3rem] shadow-2xl dark:shadow-white/10 border border-white/30 dark:border-white/10 relative flex flex-col transition-colors duration-500">
                        ...
                        >
                                    ...
                                    <Geographies geography={geoUrl}>
                                        {({ geographies }) =>
                                            geographies.map((geo) => {
                                                ...
                                                let fillClass =
                                                    'fill-[#E5E7EB] dark:fill-[#333333] hover:fill-[#9CA3AF] dark:hover:fill-[#444444]';

                                                if (isCompleted) {
                                                    fillClass =
                                                        'fill-blue-500 dark:fill-orange-500 hover:fill-blue-600 dark:hover:fill-orange-600';
                                                } else if (isPlanned) {
                                                    fillClass =
                                                        'fill-emerald-500 dark:fill-emerald-500 hover:fill-emerald-600 dark:hover:fill-emerald-600';
                                                }
                            ...
                            <div className="absolute bottom-4 left-4 lg:bottom-6 lg:left-6 bg-white/90 dark:bg-[#333]/90 backdrop-blur-md border border-gray-200 dark:border-white/10 px-4 py-3 lg:px-6 lg:py-4 rounded-2xl shadow-xl flex flex-col gap-2 pointer-events-none transition-colors">
                                ...
                                <div className="w-32 sm:w-40 lg:w-56 h-2 lg:h-3 bg-gray-300 dark:bg-gray-700 rounded-full overflow-hidden">
                                    <div
                                        className="h-full bg-green-500 dark:bg-orange-500 rounded-full shadow-[0_0_10px_rgba(34,197,94,0.5)] transition-all duration-1000"
                            ...
                            <div className="absolute bottom-4 right-4 lg:bottom-6 lg:right-6 flex flex-col gap-2 lg:gap-3">
                                <button
                                    onClick={zoomIn}
                                    className="bg-white dark:bg-[#444] hover:bg-blue-500 dark:hover:bg-orange-500 text-black dark:text-white w-10 h-10 lg:w-14 lg:h-14 flex justify-center items-center rounded-xl lg:rounded-2xl shadow-lg transition-all active:scale-95 group"
                                >
                        ...
                        <div className="bg-white/40 dark:bg-black/20 rounded-2xl lg:rounded-[2.5rem] p-4 lg:p-10 mt-6 lg:mt-8 shadow-inner border border-white/20 dark:border-white/5 flex flex-col gap-6 lg:gap-8 transition-colors">
                            <div className="flex items-center gap-2 lg:gap-3 justify-center mb-2">
                                ...
                                <h3 className="text-xl lg:text-3xl font-black text-black dark:text-white uppercase tracking-tighter text-center">
                                            ...
                                            {budgetOptions.map((option) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 flex items-center justify-center rounded-xl font-black text-sm sm:text-lg transition-all shadow-sm ${
                                                            isActive
                            ...
                            <div className="flex items-center gap-2 justify-center mt-2 opacity-50">
                                ...
                                <p className="text-[10px] lg:text-xs font-sans font-bold text-black dark:text-white italic text-center">
                                    Użyj przycisków wyżej, by wyselekcjonować konkretne podróże na mapie.
                                </p>
                            </div>
                        </div>
                    </div>
