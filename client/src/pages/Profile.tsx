...
import { User, Shield, BarChart3, Settings, FileText, Camera, Award, CheckCircle2 } from 'lucide-react';
...
const ProfileRow = ({
    label,
    value,
    valueBg = 'bg-white/50 dark:bg-black/20',
    centerValue = false,
}: {
    label: string;
    value: string;
    ...
    <div className="flex w-full mb-3 lg:mb-4 shadow-sm font-sans text-xs lg:text-base rounded-xl overflow-hidden border border-white/20 dark:border-white/5">
        ...
        <div
            className={`${valueBg} text-black dark:text-white font-bold px-3 py-2 lg:px-4 lg:py-3 flex-1 flex items-center transition-colors duration-500 ${centerValue ? 'justify-center' : ''}`}
        >
            {value}
        </div>
    </div>
...
const Profile = () => {
    ...
    const handleDeleteAccount = async () => {
        const isConfirmed = window.confirm(
            'Czy na pewno chcesz bezpowrotnie usunąć swoje konto i wszystkie zaplanowane podróże? Tej operacji nie można cofnąć.',
        );
    ...
    const savePreferences = async () => {
        try {
            await api.put('/users/preferences', userPreferencesForm);
            updateUser(userPreferencesForm);
    ...
    const handleInterestToggle = (interest: string) => {
        let current = userPreferencesForm.interests === '-' ? [] : userPreferencesForm.interests.split(', ');
        if (current.includes(interest)) {
            current = current.filter((i) => i !== interest);
        } else if (current.length < 3) {
            current.push(interest);
        }
        setUserPreferencesForm({ ...userPreferencesForm, interests: current.length > 0 ? current.join(', ') : '-' });
    };
    ...
    const renderButtons = (field: 'pace' | 'diet' | 'transport', options: string[]) => (
        <div className="flex flex-wrap gap-2 mt-1">
            {options.map((opt) => (
                <button
                    key={opt}
                    onClick={() => setUserPreferencesForm({ ...userPreferencesForm, [field]: opt })}
                    className={`px-3 py-1.5 lg:px-4 lg:py-2 rounded-xl text-xs lg:text-sm font-bold transition-all shadow-sm ${userPreferencesForm[field] === opt ? 'bg-[#6584e0] dark:bg-orange-500 text-white scale-105' : 'bg-white/80 dark:bg-[#333] text-gray-700 dark:text-gray-300 hover:bg-white dark:hover:bg-[#444]'}`}
                >
                    {opt}
                </button>
            ))}
    ...
    if (!user) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center min-h-screen text-center p-4">
                ...
                <Link
                    to="/login"
                    className="bg-[#6584e0] dark:bg-orange-500 text-white font-bold py-3 px-6 rounded-xl hover:opacity-90"
                >
            ...
            <section className="flex-1 flex flex-col justify-center items-center px-4 sm:px-8 py-20 lg:py-40 min-h-[calc(100dvh-140px)] lg:min-h-screen relative transition-all duration-500">
                <RevealOnScroll>
                    <div className="bg-gray-200/80 dark:bg-[#262626]/95 backdrop-blur-lg w-full max-w-6xl lg:max-w-[1500px] min-h-[75vh] p-6 py-10 sm:p-10 lg:p-14 rounded-3xl lg:rounded-[3rem] shadow-2xl dark:shadow-white/10 border border-white/30 dark:border-white/10 relative flex flex-col transition-colors duration-500">
                        <div
                            className={`grid grid-cols-1 ${isEditing || isEditingPrefs ? 'lg:grid-cols-1 place-items-center' : 'lg:grid-cols-3'} gap-6 lg:gap-12 h-full transition-all duration-700 ease-in-out`}
                        >
                            {!isEditingPrefs && (
                                <div
                                    className={`bg-white/40 dark:bg-black/20 rounded-3xl lg:rounded-[2.5rem] p-6 lg:p-10 shadow-inner border border-white/20 dark:border-white/5 flex flex-col items-center justify-between transition-all duration-500 hover:scale-[1.01] ${isEditing ? 'w-full max-w-2xl' : 'w-full'}`}
                                >
                                    <div className="w-full flex flex-col items-center">
                                        <div className="flex items-center gap-3 mb-6 lg:mb-8">
                                            <Shield className="w-6 h-6 lg:w-8 lg:h-8 text-blue-500 dark:text-orange-500" />
                                            <h2 className="text-3xl lg:text-5xl font-black text-black dark:text-white tracking-tight">
                                                Profil
                                            </h2>
                                        </div>
                                                    ...
                                                    <>
                                                        ...
                                                        >
                                                            {isUploading ? (
                                                                ...
                                                                <>
                                                                    <Camera className="w-6 h-6 lg:w-8 lg:h-8 mb-1" />{' '}
                                                                    Wybierz zdjęcie
                                                                </>
                                                            )}
                                                        </button>
                                        ...
                                        <div className="w-full flex flex-col items-center">
                                            ...
                                            <div className="w-full mt-2 lg:mt-4 space-y-4 lg:space-y-6">
                                                ...
                                                <ProfileRow
                                                    ...
                                                    value={longestTripDays > 0 ? `${longestTripDays} dni` : '-'}
                                                    centerValue={true}
                                                />
                                            </div>
                                            ...
                                            <Link
                                                to="/map"
                                                className="mt-6 lg:mt-8 p-4 lg:p-6 bg-blue-500/10 dark:bg-orange-500/10 border border-blue-500/20 dark:border-orange-500/20 rounded-2xl lg:rounded-3xl text-center hover:bg-blue-500/20 dark:hover:bg-orange-500/20 transition-all group w-full"
                                            >
                                                ...
                                                <p className="font-sans text-sm lg:text-base font-bold text-black dark:text-white">
                                                    Zobacz swoje odznaki na mapie świata
                                                </p>
                                            </Link>
                                        </div>
                                    ...
                                    <div className="bg-white/40 dark:bg-black/20 rounded-3xl lg:rounded-[2.5rem] p-6 lg:p-10 shadow-inner border border-white/20 dark:border-white/5 flex flex-col items-center justify-between transition-all hover:scale-[1.01]">
                                        <div className="w-full flex flex-col items-center">
                                            <div className="flex items-center gap-3 mb-6 lg:mb-8">
                                                ...
                                                <h2 className="text-3xl lg:text-5xl font-black text-black dark:text-white tracking-tight text-center">
                                                    Preferencje
                                                </h2>
                                            </div>
                                        ...
                                        <div className="w-full mt-6 lg:mt-8 flex flex-col gap-3 lg:gap-4">
                                            <button
                                                ...
                                                className="w-full bg-[#6584e0] dark:bg-orange-500 hover:bg-blue-600 dark:hover:bg-orange-600 text-white font-black py-3 lg:py-4 rounded-xl lg:rounded-2xl text-lg lg:text-xl shadow-lg transition-all hover:scale-105"
                                            >
                                                Zmień preferencje
                                            </button>
                                            <button
                                                onClick={handleDeleteAccount}
                                                className="w-full bg-red-600 hover:bg-red-700 text-white font-black py-3 lg:py-4 rounded-xl lg:rounded-2xl text-lg lg:text-xl shadow-lg transition-all hover:scale-105 opacity-80 hover:opacity-100"
                                            >
                                                Usuń konto
                                            </button>
                                        </div>
                            ...
                            <div className="absolute left-[-9999px] top-[-9999px]">
                                <div
                                    ref={pdfRef}
                                    className="w-[800px] bg-[#f8fafc] p-12 text-[#000000] font-sans relative overflow-hidden"
                                >
                                    ...
                                    <div className="flex justify-between items-end border-b-2 border-[#e5e7eb] pb-6 mb-8 relative z-10">
                                        <div>
                                            <h1 className="text-4xl font-black text-[#6584e0] tracking-tighter">
                                                Travel-Planner
                                            </h1>
                                            <p className="text-[#6b7280] font-bold mt-1 tracking-widest uppercase text-sm">
                                                Osobisty Raport Podróżnika
                                            </p>
                                        </div>
                                    ...
                                    <div className="flex items-center gap-8 bg-[#ffffff] p-6 rounded-3xl border border-[#e5e7eb] mb-8 relative z-10">
                                        ...
                                        <div className="flex-1">
                                            <h2 className="text-3xl font-black text-[#1f2937]">
                                                {user?.nickname || 'Podróżnik'}
                                            </h2>
                                            <p className="text-[#6b7280] font-medium mb-3">{user?.email}</p>
                                            <div className="flex gap-6 text-sm">
                                                <span className="bg-[#f3f4f6] px-3 py-1 rounded-full font-bold text-[#4b5563]">
                                                    Wiek: {calculateAge(user?.dateOfBirth)} lat
                                                </span>
                                                <span className="bg-[#f3f4f6] px-3 py-1 rounded-full font-bold text-[#4b5563]">
                                                    Z nami od: {formatDate(user?.createdAt)}
                                                </span>
                                            </div>
                                        </div>
                                    </div>

                                    <h3 className="text-xl font-black text-[#1f2937] mb-4 flex items-center gap-2 relative z-10">
                                        <BarChart3 color="#f97316" /> Twoje Osiągnięcia
                                    </h3>
                                    <div className="grid grid-cols-3 gap-4 mb-8 relative z-10">
                                        <div className="bg-[#ffffff] p-5 rounded-2xl border border-[#e5e7eb] text-center">
                                            <p className="text-[#6b7280] font-bold text-sm uppercase tracking-wider mb-1">
                                                Odkryte Kraje
                                            </p>
                                        ...
                                        <div className="bg-[#ffffff] p-5 rounded-2xl border border-[#e5e7eb] text-center">
                                            ...
                                            <p className="text-4xl font-black text-[#6584e0]">
                                                {longestTripDays > 0 ? `${longestTripDays} dni` : '-'}
                                            </p>
                                        </div>
                                    </div>

                                    <h3 className="text-xl font-black text-[#1f2937] mb-4 flex items-center gap-2 relative z-10">
                                        <Settings color="#f97316" /> Profil Podróżnika
                                    </h3>
                                    <div className="bg-[#ffffff] p-6 rounded-3xl border border-[#e5e7eb] grid grid-cols-2 gap-6 relative z-10">
                                        <div>
                                            <p className="text-[#6b7280] font-bold text-xs uppercase tracking-wider mb-1">
                                                Tempo zwiedzania
                                            </p>
                                            ...
                                            <div className="flex flex-wrap gap-2 mt-1">
                                                {user?.interests && user.interests !== '-' ? (
                                                    user.interests.split(', ').map((interest) => (
                                                        <span
                                                            key={interest}
                                                            className="bg-[#ffedd5] text-[#c2410c] font-bold px-2 py-1 rounded-md text-xs"
                                                        >
                                                            {interest}
                                                        </span>
                                                    ))
                                    ...
                                    <div className="mt-12 pt-6 border-t border-[#e5e7eb] text-center relative z-10">
                                        <p className="text-[#9ca3af] font-bold text-xs">
                                            Aplikacja stworzona, by zamieniać marzenia w gotowe plany.
                                        </p>
                                ...
                                <div className="bg-white/40 dark:bg-black/20 rounded-3xl lg:rounded-[2.5rem] p-6 lg:p-10 shadow-inner border border-white/20 dark:border-white/5 flex flex-col items-center justify-between w-full max-w-3xl transition-all duration-500">
                                    <div className="w-full flex flex-col items-center">
                                        ...
                                        <div className="w-full flex flex-col gap-6 font-sans">
                                            <div>
                                                <label className="text-sm font-black text-gray-700 dark:text-gray-300 ml-2">
                                                    Tempo zwiedzania
                                                </label>
                                            ...
                                            <div>
                                                ...
                                                <div className="flex flex-wrap gap-2 mt-1">
                                                    {[
                                                        ...
                                                        const isSelected =
                                                            userPreferencesForm.interests.includes(interest);
                                                        return (
                                                            <button
                                                                key={interest}
                                        ...
                                        <div className="w-full flex gap-4 mt-10">
                                            <button
                                                ...
                                                className="w-1/3 bg-gray-500 hover:bg-gray-600 text-white font-black py-3 lg:py-4 rounded-xl lg:rounded-2xl shadow-lg transition-all"
                                            >
                                                Anuluj
                                            </button>
                                            <button
                                                onClick={savePreferences}
                                                className="w-2/3 bg-green-500 hover:bg-green-600 text-white font-black py-3 lg:py-4 rounded-xl lg:rounded-2xl shadow-lg transition-all flex items-center justify-center gap-2"
                                            >
