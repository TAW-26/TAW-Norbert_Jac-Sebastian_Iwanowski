...
const MyTrips = () => {
    ...
    const handleDeleteTrip = async (tripId: number, destination: string) => {
        const isConfirmed = window.confirm(
            `Czy na pewno chcesz bezpowrotnie usunąć swoją podróż do miejsca: ${destination}?`,
        );
    ...
    const SortButton = ({ type, icon: Icon, label }: { type: string; icon: any; label: string }) => {
        ...
        return (
            <button
                ...
                className={`flex items-center gap-3 px-6 py-2.5 rounded-xl font-sans font-bold transition-all duration-300 ${
                    isActive
            ...
            >
                ...
                <span>{label}</span>
                <div
                    className={`flex items-center transition-all duration-500 ease-in-out ${isActive ? 'opacity-100 w-4' : 'opacity-0 w-0 overflow-hidden'} ${sortOrder === 'desc' ? 'rotate-180' : 'rotate-0'}`}
                >
                    ...
                    <div className="bg-gray-200/80 dark:bg-[#262626]/95 backdrop-blur-lg w-full max-w-4xl lg:max-w-[1500px] lg:min-h-[70vh] p-6 py-10 sm:p-10 lg:p-14 rounded-3xl lg:rounded-[3rem] shadow-2xl dark:shadow-white/10 border border-white/30 dark:border-white/10 relative flex flex-col transition-colors duration-500">
                        <div className="mb-10">
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                                ...
                                >
                                    <SlidersHorizontal
                                        className={`w-5 h-5 transition-transform duration-300 ${isSortOpen ? 'rotate-180' : ''}`}
                                    />
                                    Sortuj wyniki
                                    <ChevronDown
                                        className={`w-5 h-5 transition-transform duration-300 ${isSortOpen ? 'rotate-180' : ''}`}
                            ...
                            <div
                                className={`overflow-hidden transition-all duration-500 ease-in-out ${isSortOpen ? 'max-h-96 md:max-h-40 mt-6 opacity-100' : 'max-h-0 opacity-0'}`}
                            >
                                <div className="p-4 bg-white/30 dark:bg-black/20 rounded-[2rem] border border-white/20 dark:border-white/5 flex flex-wrap justify-center gap-4">
                                    <SortButton
                                        ...
                                        icon={History}
                                        label={
                                            activeSort === 'added' && sortOrder === 'asc' ? 'Najstarsze' : 'Najnowsze'
                                        }
                                    />
                        ...
                        {isLoading ? (
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center py-12 px-4 text-center">
                                ...
                                <h3 className="text-3xl lg:text-4xl font-black text-black dark:text-white mb-4">
                                    Nic tu jeszcze nie ma!
                                </h3>
                                <p className="text-lg text-gray-600 dark:text-gray-400 mb-8 max-w-md font-medium">
                                    Twoja historia podróży jest pusta. Pora to zmienić i zaplanować wymarzoną wycieczkę
                                    z pomocą naszej sztucznej inteligencji.
                                </p>
                                <Link
                                    to="/plan"
                                    className="bg-[#6584e0] dark:bg-orange-500 hover:bg-blue-600 dark:hover:bg-orange-600 text-white font-black py-4 px-8 text-xl lg:text-2xl rounded-2xl transition-all shadow-xl hover:scale-105 flex items-center gap-3"
                                >
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center py-12 px-4 text-center">
                                ...
                                <button
                                    ...
                                    className="bg-blue-500 dark:bg-orange-500 hover:bg-blue-600 dark:hover:bg-orange-600 text-white font-bold py-3 px-6 rounded-xl transition-all shadow-md"
                                >
                            ...
                            <>
                                ...
                                <div
                                    className="flex-1 flex flex-col gap-5 overflow-y-auto pr-2 max-h-[50vh] lg:max-h-[55vh] custom-scrollbar"
                                    ...
                                    {processedTrips.map((trip) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="text-2xl lg:text-4xl font-black text-black dark:text-white transition-colors text-center lg:text-left">
                                                    {trip.destination}
                                                </div>
                                                <div className="font-sans text-lg lg:text-2xl font-semibold text-gray-700 dark:text-gray-300 text-center transition-colors">
                                                ...
                                                <div className="flex justify-center">
                                                    <div
                                                        className={`w-10 h-10 lg:w-14 lg:h-14 flex items-center justify-center rounded-xl lg:rounded-2xl font-sans font-black text-lg lg:text-2xl text-white shadow-lg transition-transform ${budget.color}`}
                                                    >
                                                        {budget.label}
                                                    </div>
                                                </div>
                                                <div className="flex flex-row lg:flex-row gap-3 w-full lg:w-auto justify-center lg:justify-end mt-4 lg:mt-0">
                                                    <Link
                                                        ...
                                                        title="Zobacz"
                                                        className="p-3 bg-blue-500 hover:bg-blue-600 text-white rounded-xl transition-all shadow-md hover:scale-110"
                                                    >
                                ...
                                <div className="mt-12 flex justify-center">
                                    <Link
                                        to="/plan"
                                        className="bg-[#6584e0] dark:bg-orange-500 hover:bg-blue-600 dark:hover:bg-orange-600 text-white font-black py-4 px-8 lg:py-5 lg:px-14 text-lg lg:text-2xl rounded-2xl lg:rounded-[2rem] transition-all shadow-2xl hover:scale-105 flex items-center gap-3 group text-center w-full sm:w-auto justify-center"
                                    >
