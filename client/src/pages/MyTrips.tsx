...
import { ArrowRight, Search, SlidersHorizontal, Trash2, Edit3, Eye, Calendar, SortAsc, DollarSign, ChevronDown, Clock, ArrowUp } from 'lucide-react';
...
const MyTrips = () => {
    ...
    const trips = [
        { id: 1, countryCode: 'it', destination: 'Rzym', days: '2 dni', dates: '15.04.2026 - 17.04.2026', budget: '$$', budgetColor: 'bg-orange-500' },
        { id: 2, countryCode: 'fr', destination: 'Francja', days: '3 dni', dates: '22.05.2026 - 25.05.2026', budget: '$$$', budgetColor: 'bg-red-600' },
        { id: 3, countryCode: 'pl', destination: 'Polska', days: '1 dzień', dates: '01.04.2026', budget: '$', budgetColor: 'bg-green-500' },
        { id: 4, countryCode: 'jp', destination: 'Tokio', days: '7 dni', dates: '10.09.2026 - 17.09.2026', budget: '$$$', budgetColor: 'bg-red-600' },
        { id: 5, countryCode: 'es', destination: 'Barcelona', days: '4 dni', dates: '05.06.2026 - 09.06.2026', budget: '$$', budgetColor: 'bg-orange-500' },
        { id: 6, countryCode: 'is', destination: 'Reykjavík', days: '5 dni', dates: '12.01.2027 - 17.01.2027', budget: '$$$', budgetColor: 'bg-red-600' },
        { id: 7, countryCode: 'us', destination: 'Nowy Jork', days: '10 dni', dates: '20.10.2026 - 30.10.2026', budget: '$$$', budgetColor: 'bg-red-600' },
        { id: 8, countryCode: 'gr', destination: 'Santorini', days: '6 dni', dates: '14.07.2026 - 20.07.2026', budget: '$$', budgetColor: 'bg-orange-500' }
    ];
    ...
    const SortButton = ({ type, icon: Icon, label }: { type: string, icon: any, label: string }) => {
        ...
        return (
            ...
            >
                ...
                <span>{label}</span>
                <div
                    className={`flex items-center transition-all duration-500 ease-in-out ${isActive ? 'opacity-100 w-4' : 'opacity-0 w-0 overflow-hidden'
                        ...
                        <div className="mb-10">
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                                ...
                                <button
                                    ...
                                    className={`w-full md:w-auto font-bold py-3 lg:py-4 px-10 text-lg lg:text-xl rounded-2xl transition-all shadow-lg flex items-center justify-center gap-2 group ${isSortOpen ? 'bg-blue-600 dark:bg-orange-600 text-white' : 'bg-white dark:bg-white/10 text-black dark:text-white hover:bg-gray-100 dark:hover:bg-white/20'}`}
                                >
                            ...
                            <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isSortOpen ? 'max-h-96 md:max-h-40 mt-6 opacity-100' : 'max-h-0 opacity-0'}`}>
                                <div className="p-4 bg-white/30 dark:bg-black/20 rounded-[2rem] border border-white/20 dark:border-white/5 flex flex-wrap justify-center gap-4">
                                    <SortButton type="date" icon={Calendar} label="Według daty" />
                                    <SortButton type="name" icon={SortAsc} label="Alfabetycznie" />
                                    <SortButton type="budget" icon={DollarSign} label="Według budżetu" />
                                    <SortButton type="time" icon={Clock} label="Według czasu" />
                                </div>
                            </div>
                        </div>

                        {/* TABELA DANYCH (Nagłówki) */}
                        ...
                        <div className="flex-1 flex flex-col gap-5 overflow-y-auto pr-2 max-h-[50vh] lg:max-h-[55vh] custom-scrollbar" style={{ scrollbarWidth: 'thin' }}>
                            {trips.map((trip) => (
                                <div key={trip.id} className="bg-white/40 dark:bg-black/20 rounded-2xl lg:rounded-full p-6 lg:px-10 lg:py-5 shadow-sm border border-white/20 dark:border-white/5 flex flex-col lg:grid lg:grid-cols-[140px_1fr_150px_250px_100px_180px] items-center gap-4 lg:gap-6 transition-all duration-300 hover:bg-white/60 dark:hover:bg-black/40 hover:scale-[1.01] group">
                                    <img src={`https://flagcdn.com/w160/${trip.countryCode}.png`} alt={trip.destination} className="w-20 h-12 lg:w-24 lg:h-16 object-cover rounded-xl shadow-md border border-black/5 dark:border-white/10 transition-transform group-hover:scale-110" />
                                    <div className="text-2xl lg:text-4xl font-black text-black dark:text-white transition-colors">{trip.destination}</div>
                                    <div className="font-sans text-lg lg:text-2xl font-semibold text-gray-700 dark:text-gray-300 text-center transition-colors">{trip.days}</div>
                                    <div className="font-sans text-sm lg:text-xl font-medium text-gray-600 dark:text-gray-400 text-center transition-colors">{trip.dates}</div>
                                    <div className="flex justify-center">
                                        <div className={`w-10 h-10 lg:w-14 lg:h-14 flex items-center justify-center rounded-xl lg:rounded-2xl font-sans font-black text-lg lg:text-2xl text-white shadow-lg transition-transform ${trip.budgetColor}`}>{trip.budget}</div>
                                    </div>
                                    <div className="flex flex-row lg:flex-row gap-3 w-full lg:w-auto justify-center lg:justify-end mt-4 lg:mt-0">
                                        <Link to="/trip-result" title="Zobacz" className="p-3 bg-blue-500 hover:bg-blue-600 text-white rounded-xl transition-all shadow-md hover:scale-110"><Eye className="w-5 h-5" /></Link>
                                        <button title="Edytuj" className="p-3 bg-orange-500 hover:bg-orange-600 text-white rounded-xl transition-all shadow-md hover:scale-110"><Edit3 className="w-5 h-5" /></button>
                                        <button title="Usuń" className="p-3 bg-red-600 hover:bg-red-700 text-white rounded-xl transition-all shadow-md hover:scale-110"><Trash2 className="w-5 h-5" /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
