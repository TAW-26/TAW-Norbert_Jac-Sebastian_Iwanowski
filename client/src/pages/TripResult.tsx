...
import { Save, Map as MapIcon, Loader2, ArrowLeft, Edit3, Trash2, PlusCircle, Check, CheckCircle } from 'lucide-react';
...
const TripResult = () => {
    ...
    const handleMarkAsCompleted = async () => {
        const confirm = window.confirm(
            'Czy na pewno chcesz oznaczyć tę podróż jako zrealizowaną? Spowoduje to przeniesienie jej do historii i zablokuje możliwość dalszej edycji planu.',
        );
        if (!confirm) return;
                    ...
                    <div className="bg-gray-200/80 dark:bg-[#262626]/95 backdrop-blur-lg w-full max-w-4xl lg:max-w-[1600px] lg:min-h-[75vh] p-6 py-10 lg:p-12 xl:p-16 rounded-3xl lg:rounded-[3rem] shadow-2xl dark:shadow-white/10 border border-white/30 dark:border-white/10 relative flex flex-col justify-between transition-colors duration-500">
                        <div className="text-center mb-8 lg:mb-14">
                            {id === 'new' && (
                                <div className="bg-orange-500/20 border border-orange-500 text-orange-600 dark:text-orange-400 font-bold px-6 py-3 rounded-2xl mb-6 inline-flex items-center justify-center gap-3">
                                    <span className="animate-pulse">👁️</span> To jest tylko podgląd. Zapisz plan do
                                    bazy, aby odblokować możliwość jego edycji!
                                </div>
                            )}
                            ...
                            {id !== 'new' && !trip?.isCompleted && (
                                <button
                                    ...
                                    className={`mt-6 px-6 py-3 rounded-2xl font-bold shadow-md transition-all flex items-center justify-center gap-2 mx-auto ${
                                        isEditing
                                            ? 'bg-green-500 text-white'
                                            : 'bg-white dark:bg-black/20 text-black dark:text-white hover:bg-gray-100 dark:hover:bg-white/10'
                                    }`}
                                >
                                    {isEditing ? (
                                        <>
                                            <Check className="w-5 h-5" /> Gotowe, zapiszmy to
                                        </>
                                    ) : (
                                        <>
                        ...
                        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 flex-1 items-stretch">
                            <div className="w-full lg:w-3/5 bg-white/40 dark:bg-black/20 rounded-3xl lg:rounded-[2rem] p-4 sm:p-6 lg:p-8 shadow-inner border border-white/20 dark:border-white/5 flex flex-col max-h-[500px] lg:max-h-[65vh] transition-colors">
                                <div
                                    className="flex-1 overflow-y-auto pr-2 lg:pr-4 custom-scrollbar"
                                    ...
                                    {daysArray.length === 0 ? (
                                        ...
                                        daysArray.map((day: any, dayIdx: number) => (
                                            <div key={day.id || day.dayNumber || dayIdx}>
                                                <div
                                                    className={`bg-gray-200/90 dark:bg-[#333333]/95 backdrop-blur-md rounded-full py-2 lg:py-3 mb-6 lg:mb-8 shadow-md border border-white/40 dark:border-white/10 sticky top-0 z-10 transition-colors ${(day.dayNumber || dayIdx + 1) > 1 ? 'mt-8' : ''}`}
                                                >
                                                    <h3 className="text-lg lg:text-3xl font-black text-center text-black dark:text-white uppercase tracking-wider">
                                                        📅 DZIEŃ {day.dayNumber || dayIdx + 1}
                                                    </h3>
                                                </div>
                                                        ...
                                                        {(day.dayNumber === 1 || dayIdx === 0) && (
                                                            <thead>
                                                                <tr className="border-b-2 border-gray-400/50 dark:border-gray-600/50 text-base lg:text-2xl text-black dark:text-white">
                                                                    <th className="pb-3 lg:pb-4 font-black w-[15%] pl-2 lg:pl-4">
                                                                        Godz.
                                                                    </th>
                                                                    <th className="pb-3 lg:pb-4 font-black w-[35%]">
                                                                        Atrakcja
                                                                    </th>
                                                                    <th className="pb-3 lg:pb-4 font-black w-[40%] pr-2 lg:pr-4">
                                                                        Opis / Notatka
                                                                    </th>
                                                                    {isEditing && (
                                                                        <th className="pb-3 lg:pb-4 font-black w-[10%] text-center">
                                                                            Akcja
                                                                        </th>
                                                                    )}
                                                                </tr>
                                                            ...
                                                            {(day.activities || []).map(
                                                                (activity: any, actIdx: number) => (
                                                                    <tr
                                                                        ...
                                                                        className={`border-b border-gray-400/20 dark:border-gray-700/30 hover:bg-white/20 dark:hover:bg-white/5 transition-colors ${actIdx % 2 !== 0 ? 'bg-gray-400/5 dark:bg-white/5' : ''}`}
                                                                    >
                                                                        <td className="py-4 lg:py-6 pl-2 lg:pl-4 align-top w-[15%]">
                                                                            {isEditing ? (
                                                                                ...
                                                                                <span className="font-bold text-blue-500 dark:text-orange-500">
                                                                                    {activity.startTime || '---'}
                                                                                </span>
                                                                            )}
                                                                        </td>
                            ...
                            <div className="w-full lg:w-2/5 bg-white/40 dark:bg-black/20 rounded-3xl lg:rounded-[2rem] p-6 lg:p-10 shadow-inner border border-white/20 dark:border-white/5 flex flex-col transition-colors">
                                ...
                                <div className="flex-1 flex flex-col justify-center space-y-6 lg:space-y-10">
                                    <div className="flex items-center gap-4 lg:gap-5 group">
                                        ...
                                        <div>
                                            <p className="text-xs lg:text-sm font-sans font-bold text-gray-500 dark:text-gray-400 uppercase tracking-tighter">
                                                Budżet ({trip?.budgetLevel || '-'})
                                            </p>
                                    ...
                                    <div className="flex items-center gap-4 lg:gap-5 group">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-lg lg:text-3xl font-black text-black dark:text-white">
                                                {daysArray.reduce(
                                                    (total: number, day: any) => total + (day.activities?.length || 0),
                                                    0,
                                                )}{' '}
                                                miejsc
                                            </p>
                        ...
                        <div className="mt-8 lg:mt-16 flex flex-col sm:flex-row justify-center items-center gap-4 lg:gap-10">
                            {id === 'new' ? (
                                <button
                                    ...
                                    disabled={isSaving}
                                    className={`bg-[#6584e0] dark:bg-orange-500 hover:bg-blue-600 dark:hover:bg-orange-600 text-white font-black py-4 px-6 lg:py-5 lg:px-12 text-lg lg:text-3xl rounded-2xl transition-all shadow-xl hover:scale-105 flex items-center justify-center gap-3 group w-full sm:w-auto ${isSaving ? 'opacity-50 cursor-not-allowed' : ''}`}
                                >
                                    {isSaving ? (
                                ...
                                <Link
                                    to="/trips"
                                    className="bg-gray-500 dark:bg-gray-600 hover:bg-gray-600 dark:hover:bg-gray-700 text-white font-black py-4 px-6 lg:py-5 lg:px-12 text-lg lg:text-3xl rounded-2xl transition-all shadow-xl hover:scale-105 flex items-center justify-center gap-3 group w-full sm:w-auto"
                                >
                                    <ArrowLeft className="w-5 h-5 lg:w-8 lg:h-8 group-hover:-translate-x-2 transition-transform" />{' '}
                                    Wróć do listy
                                </Link>
                            )}
                            ...
                            {id !== 'new' && isPastTrip && !trip?.isCompleted && !isEditing && (
                                ...
                                >
                                    {isCompleting ? (
                                        ...
                                        <CheckCircle className="w-5 h-5 lg:w-8 lg:h-8 group-hover:scale-110" />
                                    )}
                                    Podróż zrealizowana! ✅
                                </button>
                            )}
